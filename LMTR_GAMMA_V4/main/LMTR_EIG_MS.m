function [x,f,outinfo] = LMTR_EIG_MS(fun,x0,params)
% LMTR_EIG_MS - limited memory trust-region algorithm EIG-MS. 
%
% For details, see: 
...
%
% [x,f,outinfo] = LMTR_EIG_MS(fun,x0,params) finds a minimizer x of function 
% defined by a handle fun starting from x0 using EIG-MS. f is the function 
% value defined at x.
...
while (mflag==1)  
    ...
    if ~Nflag   
        ...
        while (ratio<=tau0) 
            ...
            msit=1;  % counter for inner MS iterations
            while (abs(nst-trrad)>mstol) && (msit<=MSmaxit) && (nst>trrad+mstol)
                ssprim=norm(vpar(1:rankV)./sqrt(sigma + lam(1:rankV)))^2+...
                        ngperp2*alpha^3;
                ...
                nst=sqrt(alpha^2*ngperp2+norm(vpar(1:rankV))^2);
                msit=msit+1;
            end
            ...
            if abs(ared)/af<ftol
                ...
                ratio=1;
            elseif ared<0
                % predicted reduction -0.5*(-g's+sigma*||s||^2)
                pred=-0.5*(alpha*(ng^2+p(1:rankV)'*Vg(lindepV))+sigma*nst^2);                
                ratio=ared/pred;                
            else
                ratio=0;
            end                        
