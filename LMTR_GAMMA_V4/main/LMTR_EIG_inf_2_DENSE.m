function [x,f,outinfo,gamms] = LMTR_EIG_inf_2_DENSE(fun,x0,params)
% LMTR_EIG_inf_2_DENSE - limited memory trust-region algorithm EIG(inf,2). 
%
% J.B 11/16/16, Function extension for the case that a dense initial matrix
% is used: B_0 = P SIG P', where SIG := Diagonal, P:= Eigenvectors of
% Compact Quasi-Newton matrix.
...
%               0 - no display.
%   gamma_opt - option to toggle between different ways to compute
%   gamma_perp :
%                   1 - multiple of identity : gamma_perp = y'y / s'y
%                   (default)
%                   2 - dense : gamma_perp = max( y'y / s'y  )
%                   3 - dense : gamma_perp = s'y / s's
%   gamma_store - Store flag for the gamma values computed.
%
...
%
%   gamms   - store of gamma values, in particular for the dense initial
%               matrix.
% 
...
% J.B 11/17/16 initialize for max. gamma
...
% J.B 11/29/16 No check for the trust-region radius. This variable is to
% store the number of times low rank V matrices are generated.
% if ns~=1
%     tract=tract+1;
% end
...
while (mflag==1)    
    ...
    %% J.B 11/17/16
    % Dense B_0 matrix requires computation of eigenvalues every iteration.
...
%         ared=ft-f;
%         if abs(ared)/af<ftol
%             ratio=1;  % relative actual reduction is too small        
%         elseif ared<0
...
%     end  % checking quasi-Newton step
    ...
    %if ~Nflag
        ...
        %% J.B 11/17/16 Compute gamma_perp
        ...
        while (ratio<=tau0) 
            ...
            %% J.B 11/17/16 Compute alpha
            %alpha=min(idelta,trrad/ngperp);
            if ngperp*igamma_p <= trrad; alpha = igamma_p; else alpha = trrad/ngperp; end;
            
            %ns_perp=min(trrad,ngperp/delta);  % L2 norm of s_perp
            ns_perp=min(trrad,ngperp*igamma_p);  % L2 norm of s_perp
            ...
            elseif ared<0               
                pred=vpar(1:rankV)'*(gpar(1:rankV)+0.5*lam(1:rankV).*vpar(1:rankV))...
                    +(alpha^2*gamma_p/2-alpha)*ngperp^2;                
                ratio=ared/pred;                
            else
                ratio=0;
            end              
