function [x,f,outinfo] = LBFGS_TR(fun,x0,params)
% LBFGS_TR - limited memory line-search algorithm L-BFGS that takes a trial 
%            step along the quasi-Newton direction inside the trust region. 
%
% For details, see: 
...
% 
% [x,f,outinfo] = LBFGS_TR(fun,x0,params) finds a minimizer x of function defined 
% by a handle fun starting from x0 using L-BFGS. f is the function value 
% defined at x.
...
%% Initialization: line-search along normalized steepest descent
it=1;
d=-g0/ng;
x=x0;
f=f0;
...
while (mflag==1)
    ...
    sN=-gamma*g+Y(:,maskV)*(gamma*w) - S(:,maskV)*w2;
    st=sN;
    
    % Sufficient decrease search along the quasi-Newton direction
    nsN=norm(sN);
    nst=nsN;
    gsN=-gamma*ng^2+gamma*w'*Yg(1:numsy)-w2'*Sg(1:numsy);
    predN=gsN/2;    
    trrad_old=trrad;
    af=max(1,abs(f));   
    ...
    if ~Nflag
        %% Full quasi-Newton step is rejected, run line search
        tract=tract+1;
        ratio=0;        
        trit=0;
        while (ratio<=tau0) 
